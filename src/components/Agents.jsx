...
const columns = (clickHandler, getAllUsers) => [
  {
    title: '#',
    key: 'userId',
    render: (text) => {
...
const Agents = ({location}) => {
  ...
  const fetchData = async (controller) => {
    ...
    try {
      ...
      const filterUrl = `${url}?skip=${start}&limit=${filterFetchLimit}&role=agent`;
      const data = {
        url:
          filter !== true
            ? `${url}?skip=${start}&limit=${fetchLimit}&role=agent`
            : filterUrl,
        method: 'GET',
        ...
        <React.Fragment>
          ...
          {isLoading === true ? (
            ...
            <div className="usersTable">
              <Table
                dataSource={allUsers}
                columns={columns(detailsHandler, allUsers)}
                rowKey="userId"
                pagination={false}
              />
              <TablePagination
